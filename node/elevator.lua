...
local function command_handler(cmd) -- Decodes the rednet message and returns the seperated variables
    ...
    local cmd_floor = decoded["floor"]
    return {cmd_id,cmd_command,cmd_speed,cmd_floor}
end
...
while true do
    ...
    if command_logic_process(cmd_res[1]) then
        ...
        elseif cmd_res[2] == "floor" then
            for z,y in pairs(floors) do
                if cmd_res[4] == z then
                    goto_floor = y
                end
            end
            local calcs = calc_pos(goto_floor)
            move(calcs[1],calcs[2],move_speed)
            pos_file_util("set",goto_floor)
